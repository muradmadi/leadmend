...
from app.schemas.lead import LeadWebhookPayload, Lead
from app.services.validator import normalize_payload, is_garbage
...
async def process_lead(
    ...
    try:
        ...
        if is_garbage(payload):
            log.warning("Pipeline short-circuited: Garbage lead detected", email=payload.email)
            score = 0
            is_duplicate = False
            enriched_data = {
                "industry": "Unknown", 
            ...
            routing_result = await route_lead(score, {
                "email": payload.email,
        ...
        log.info("Duplicate check complete", is_duplicate=is_duplicate, match_type=dup_check.get("match_type"))
        ...
        if is_duplicate and existing_id:
            ...
            if existing_lead:
                ...
                if not existing_lead.company_name or existing_lead.company_name == "Unknown":
                    existing_lead.company_name = enriched_data.get("company_name") or payload.company
                if not existing_lead.industry or existing_lead.industry == "Unknown":
                    existing_lead.industry = enriched_data.get("industry")
                if not existing_lead.company_size or existing_lead.company_size == "Unknown":
                    existing_lead.company_size = enriched_data.get("company_size")
                if not existing_lead.company_domain or existing_lead.company_domain == "Unknown":
                    existing_lead.company_domain = company_domain
                
                await db.commit()
                log.info("Existing lead record healed with new enrichment data", lead_id=existing_id)
            lead_id = existing_id
        else:
