...
async def test_route_lead_high_tier():
    ...
    with patch("app.services.router.send_slack_notification", new_callable=AsyncMock) as mock_slack, \
         patch("app.services.router.send_to_crm", new_callable=AsyncMock) as mock_crm:
...
async def test_route_lead_no_webhook_logs(caplog):
    ...
    with patch("app.services.router.settings") as mock_settings, \
         patch("app.services.router.send_to_crm", new_callable=AsyncMock) as mock_crm, \
         patch("app.services.slack_notifier.httpx.AsyncClient.post") as mock_post:
        ...
        assert "routed_to" not in plan # Based on my implementation it's "slack_notified"
        assert plan["slack_notified"] == False
